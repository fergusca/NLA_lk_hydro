...
# REMOVE LEGEND
map_conus<-map_conus + theme(legend.position="none")

# REVISED MINIMALLY DISTURBED AND FULL CONUS MAPS
tiff(filename="~/NLA_hydro/NLA_hydro_driver/figures/map_compile_min_hydro_all_conus.tiff",
     width=7.5, height=10, units="in", res=400)
grid.arrange(arrangeGrob(map_min + theme(legend.position = "none"),
                         map_conus + theme(legend.position = "none"), nrow = 2),
             #nrow=2),

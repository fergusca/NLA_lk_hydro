...
dat %>%
  group_by(ECOREG_rev) %>%
  summarize(mean_vert = mean(VertDD_use, na.rm=TRUE),
            med_vert = median(VertDD_use, na.rm=TRUE),
            mean_horiz = mean(HorizDD_use, na.rm=TRUE),
            med_horiz = median(HorizDD_use, na.rm=TRUE),
            mean_EI = mean(E_I,na.rm=TRUE),
            med_EI = median(E_I, na.rm=TRUE))
...
#####################
## Coastal Plains n= 189
#########
cstp<-dat %>%
  filter(ECOREG_rev=="Coastal Plains")

summary(cstp$bffFlat_grad)
summary(west$bffFlat_grad)
#######################
...
low_hydrap_summary <- dat %>%
  group_by(hydro_dist) %>%
  summarise_at(c("VertDD_use","HorizDD_use","E_I"),list(mean,median,sd,min,max),na.rm=TRUE
  )
...
###########################
# Low HydrAP - Drought effects on vertical decline by lake hydrology (E:I class)
vert_phdi_hydrap_low<-ggplot(low_hydrap, aes(x= PHDI, y=VertDD_use, color=lk_hydro_iso))+ #, shape=hydrap_bin))+
  scale_y_continuous(trans="log10",limits=c(NA,5),labels=function(x) format(x,scientific = FALSE))+
  geom_point(alpha=0.5) +
  geom_smooth(method=lm, se=FALSE, fullrange=FALSE)+
  #xlim(NA,350)+
  facet_wrap(~lk_hydro_iso, ncol=5)+
  #scale_color_manual(name="HydrAP class", values=c("#4682b4", "#b4af46","#b4464b"),na.value= "#999999")+ #"#2c7bb6", "#fdae61","#d7191c"
  stat_cor(aes(color=lk_hydro_iso, family="RMN"), label.x=-4)+ 
  theme_bw(base_size=12)+
  theme(plot.title = element_text(family = "RMN", face="plain", size = 12, hjust=0.5),
        axis.text.x = element_text(family = "RMN", angle=45, hjust=1),
        axis.text.y = element_text(family = "RMN"),
